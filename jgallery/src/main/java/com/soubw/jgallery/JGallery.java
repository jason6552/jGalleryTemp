...
public class JGallery extends FrameLayout implements ViewPager.OnPageChangeListener, OnJGalleryPageSelectedListener {
    ...
    private boolean autoPlay = false;    // 自动轮播
    private boolean autoLoop = true;    // 循环轮播
...
//                startAutoPlayTack(true);
    ...
    private final Runnable autoPlayTack = new Runnable() {
        ...
        public void run() {
            /**
             * 解决视频播放未完成直接切换核心：1.判断当前元素类型，视频则停止轮询，播放完成后 再开始开始轮询
             */
            if (autoPlay) {
                if (jGalleryPagerAdapter.getCount() > 1) {
                    ///////////////////////////////////////////////
                    if(jGalleryPagerAdapter.getItemType(currentPos) == DataType.NET_VIDEO ||
                            jGalleryPagerAdapter.getItemType(currentPos) == DataType.LOCAL_VIDEO){
                        return;
                    }
                    ///////////////////////////////////////////////
    ...
    public void onMessageEvent(MediaCompletion event) {
        currentPos = currentPos == jGalleryPagerAdapter.getCount() - 1 ? autoLoop ? 0 : -1 : currentPos + 1;
        if (currentPos == -1)
            return;
        if (currentPos == 0) {
